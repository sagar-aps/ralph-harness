...
- **Identities**: <!-- Manager and orchestrator identities (GitHub App ids / accounts), where credentials live, deviations from the standard lookup ($RALPH_IDENTITY_WRAPPER → executable .agents/ralph/identity.sh → ambient gh auth), the boot check that names the active identity, and whether branch protection actually requires an approval. Omit if no identity split exists — then the Manager runs permanently in fallback mode. -->
